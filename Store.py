...
# Bot does not include functionality to add card details, these should be set up beforehand.

class Store:
    def __init__(self, username, password): # Constructor of the class, when called, self represents the instance, giving access to class attributes/methods
        """ Initialises Bot with class-wide attributes. """
        self.username = username  # Makes attributes class-wide
    ...
    def buyProduct(self):
        ...
        isAvailable = self.isProductAvailable(price_indicator)  # Localise returned instance of class to setup for price comparison
        ...
        # Buy Now
        if isAvailable <= Globals.PRICE_LIMIT:  # isProductAvailable function will have returned "Product Available" here
            ...
            if price_indicator == 1:
                ...
                window_h = driver.execute_script('return window.innerHeight')  #Height of the current viewport
                desired_y = element_y - (window_h / 2)  # Scroll the element to the top of the page, minus 1/2 the width of the current viewport
                ...
                # Place Order
                place_order = driver.find_element_by_name('placeYourOrder1').text  # VERIFY
                time.sleep(randint(int(Globals.WAIT_TIME / 2), Globals.WAIT_TIME))
    ...
    def signIn(self):
        ...
        for i in range(0, len(user_val)):
            ...
            time.sleep(0.1)
        username_elem.send_keys(Keys.RETURN)  # Simulation of pressing Return key
        time.sleep(1)
        try:
        ...
        for i in range(0, len(pass_val)):
            ...
            password_elem.send_keys(pass_string)  # Send each letter to the browser
            time.sleep(0.1)
        password_elem.send_keys(Keys.RETURN)  # Simulation of pressing Return key
        time.sleep(1)
        try:
            error_display = driver.find_element_by_id('auth-error-message-box').is_displayed()
        except:
    ...
    def isProductAvailable(self, indicator):
        ...
        if indicator == 1:
            ...
            available = driver.find_element_by_class_name('a-color-price').text  # Using CSS, will return "Currently Unavailable" or Product Price
            if available == 'Currently unavailable.':
                print(f'***** AVAILABLE: {available}')  # Format string to value of available string
...
if __name__ == '__main__':  # Call the main method and instantiate class attributes
